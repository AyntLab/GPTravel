...
📍 Runs in the notification area — no screen occupation.
Just input your base prompt once, and the AI automatically talks to you based on GPS position and time.
（通知欄常駐式なので画面を占有しません。プロンプトを入力しておくだけで、GPSと時刻からAIが自動的に語りかけます。）

🗺️ Export travel logs as KML or CSV.
You can view on Google Maps when and where the AI talked to you.
（期間指定でKML／CSV形式のログ出力が可能。Googleマップ上で、どの地点でどんな発言があったかを確認できます。）

🔑 Requires your own OpenAI API key (enter it at first launch).
（OpenAIのAPIキーが必要です。最初にご自身のキーを入力してください。）
